...
async function loadProjectData(name) {
  ...
  return new Promise(resolve => {
    console.log(
      `loadProjectData: entering Promise and waiting for ${sleepTime}ms to return ${name}`
    );
    setTimeout(() => {
      console.log(
        `loadProjectData: Promise done - returning value: ${name} after ${sleepTime}ms`
      );
...
isFinishedElement.innerHTML +=
  "Script has finished! (tasks were delegated to Promises)";
